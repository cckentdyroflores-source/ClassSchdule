...
<div class="dash-wrap">
  ...
  {{-- ── METRIC CARDS ── --}}
  {{-- Pass these from your DashboardController:
       $totalStudents, $totalTeachers, $totalSchedules, $totalRooms --}}
  <div class="metric-grid">
    <div class="metric-card">
      <div class="mc-icon blue"><i class="ti ti-school"></i></div>
      <div class="mc-label">Total students</div>
      <div class="mc-val">{{ number_format($totalStudents ?? 0) }}</div>
    ...
    <div class="metric-card">
      ...
      <div class="mc-val">{{ number_format($totalTeachers ?? 0) }}</div>
  ...
  {{-- Pass $recentSchedules (latest 5) and $totalSchedules from DashboardController --}}
  <div class="panel">
    <div class="panel-head">
      <div>
        <div class="panel-title">Recent class schedules</div>
        <div class="panel-sub">
          Latest {{ ($recentSchedules ?? collect())->count() }} of {{ number_format($totalSchedules ?? 0) }} schedules
        </div>
      </div>
      <a href="{{ route('schedules.index') }}" class="btn btn-sm">
    ...
    <div style="overflow-x:auto;">
      <table class="data-table">
        ...
        @forelse(($recentSchedules ?? []) as $i => $s)
          @php
          ...
          <tr>
            ...
            <td>
              <span class="time-val">
                {{ $s->formatted_start ?? date('h:i A', strtotime($s->start_time ?? 'now')) }}<span class="time-sep">–</span>{{ $s->formatted_end ?? date('h:i A', strtotime($s->end_time ?? 'now')) }}
              </span>
            </td>
...
{{-- ── ADD SCHEDULE MODAL (exact same fields as schedules/index.blade.php) ── --}}
...
<script>
  ...
  new Chart(document.getElementById('scheduleChart'), {
    ...
    data: {
      ...
      datasets: [{
        label: 'Classes',
        data: {!! json_encode($classesPerDay) !!},
